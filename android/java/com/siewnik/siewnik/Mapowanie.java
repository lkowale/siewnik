...
/**
 * Created by pandejo on 11.01.2018.
 */

public class Mapowanie extends ArduinoEvent {
    void init(TextView t, BluetoothSocket btS){
        super.init(t,btS);

        printLine("Mapowanie start");

        try
        {
            btSocket.getOutputStream().write("m1".toString().getBytes());
        }
        catch (IOException e)
        {
//                msg("Error");
...
////            printLine("Obrot: "+s);
    ...
    void parse(String indeks,String s1,String s2){
        if(indeks.equals("m2"))
            printLine("PWM: "+s1+" czas 1 obrotu[ms]:"+s2);
    }
}
